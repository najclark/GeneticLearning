...
public class Phenotype implements Comparable<Phenotype>, Serializable {
	...
	public static int ALL_OgtIONS = 0;
	...
	public Phenotype(double fitness, Genotype gt, String name) {
		this.gt = gt;
	...
	public static String mutateName(String name, int letters, int ogtion) {
		String[] arr = name.split("");
		for (int i = 0; i < letters; i++) {
			int index = random.nextInt(name.length() - ogtion) + ogtion;
	...
	public void setPhenotypek(Genotype gt) {
		this.gt = gt;
	}
	...
	@Override
	public int compareTo(Phenotype i2) {
	 ...
	 */
	public static Phenotype openNN(String path) {
		Phenotype ind = null;
		try {
			FileInputStream fileIn = new FileInputStream(path);
			ObjectInputStream in = new ObjectInputStream(fileIn);
			ind = (Phenotype) in.readObject();
			in.close();
			fileIn.close();
			return ind;
		} catch (IOException i) {
